...
*/
salesList = transactions.filter(transaction => transaction.type === 'sale');
var numSales = salesList.length;
...
*/
purchasesList = transactions.filter(transaction => transaction.type === 'purchase');
var numPurchases = purchasesList.length;
...
*/
cashSalesList = transactions.filter(transaction => transaction.type === 'sale' && transaction.paymentMethod === 'cash')

var numCashSales = cashSalesList.length;
...
*/
creditPurchasesList = transactions.filter(transaction => transaction.type === 'purchase' && transaction.paymentMethod === 'credit')

var numCreditPurchases = creditPurchasesList.length;
...
*/
var uniqueVendors = transactions.filter(transaction => transaction['vendor'] !== undefined).map(transaction => transaction['vendor']);
...
*/
var uniqueCustomers = transactions.filter(transaction => transaction['customer'] !== undefined).map(transaction => transaction['customer']);
...
*/
bigPurchases = transactions.filter(transaction => transaction.items.length >= 5);
var bigSpenders = bigPurchases.map(function(transaction) {return {'name': transaction['customer'], 'numItems': transaction['items'].length}});
...
*/
var firstSale = transactions.filter(transaction => transaction.type === 'sale')[0];
const reducer = (accumulator, currentValue) => accumulator + currentValue;
var sumSales = firstSale.items.map(item => item.price).reduce(reducer);
...
*/
var allPurchasesSums = [];

function purchasesSum() {
  transactions.forEach(function(transaction){
    if (transaction.type === 'purchase') {
      var amountSpent = transaction.items.map(item => item.price).reduce(reducer);
      allPurchasesSums.push(amountSpent);
    }
  })
}
purchasesSum();
var sumPurchases = allPurchasesSums.reduce(reducer);
...
function salesSum() {
  transactions.forEach(function(transaction){
    if (transaction.type === 'sale') {
      var amountSpent = transaction.items.map(item => item.price).reduce(reducer);
      allSalesSum.push(amountSpent);
    }
  })
}
salesSum();
sumSales = allSalesSum.reduce(reducer);
...
*/
var transactionsSortedBySize = transactions.map(transaction => transaction.items.length).sort();
var mostItems = transactionsSortedBySize[transactionsSortedBySize.length - 1];
...
*/
var sortedPurchases = purchasesList.sort(function (a, b) {
  return a.items.length - b.items.length;
});

var sumOfSmallestPurchase = sortedPurchases[0].items.map(item => item.price).reduce(reducer);
